...
def upsampling(
        X_df: pd.DataFrame, 
        y_series: pd.Series, 
        strategy: Literal["equal"] | Dict[str|int|float, float]  = 'equal', 
        random_state: int = 1, 
        verbose: int = 1) -> Tuple[pd.DataFrame, pd.Series]:
    
    # v02: Modified to raise an error if strategy keys do not match labels
    # and include an example of a valid strategy
    import pandas as pd
    import numpy as np
